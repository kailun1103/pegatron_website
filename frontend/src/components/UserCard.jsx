...
export default function UserCard({ user, onEdit, onDelete, onView }) {
  ...
  const imageUrl = avatar_url || fallbackAvatar
    ...
    <>
      <div className="card h-100">
        <img
          ...
          alt={`${name} avatar`}
          style={{ objectFit: 'cover', height: 160, cursor: 'pointer' }}
      ...
      <ImageModal src={showImage ? imageUrl : ''} alt={`${name} avatar`} onClose={() => setShowImage(false)} />
    </>
  )
}
